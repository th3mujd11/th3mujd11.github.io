...
function timeAgo(iso) { // function takes ISO timestamp string
...
// main page component
export default function Home() { // export default React component
  ...
  useEffect(() => { // run once on mount
    ...
    async function fetchTrack() { // inner async function to fetch
      try { // try/catch for robust error handling
        ...
        const next = { // construct a simplified track object
      ...
      } catch (err) { // handle fetch/parse errors
        console.error("Last.fm fetch error:", err); // log for debugging
        if (!cancelled) setErrors(e => ({ ...e, track: "Failed to load track" })); // set error state
      } // end try/catch
    } // end fetchTrack
    fetchTrack(); // initial fetch on mount
    const id = setInterval(fetchTrack, 30000); // refresh every 30 seconds
    return () => { cancelled = true; clearInterval(id); }; // cleanup on unmount
  ...
  useEffect(() => { // run once on mount
    ...
    async function fetchCommit() { // inner async fetch function
      try { // begin error handling
        const HEADERS = { headers: { Accept: "application/vnd.github+json", "X-GitHub-Api-Version": "2022-11-28" } }; // polite headers
        ...
        try {
          ...
          if (res.ok) { // proceed only if ok
            const events = await res.json(); // parse events JSON
            const push = events.find(e => e?.type === "PushEvent" && e?.payload?.commits?.length); // first PushEvent with commits
            if (push) { // if push event exists
              const first = push.payload.commits[push.payload.commits.length - 1]; // earliest in push
        ...
        } catch { /* ignore events errors (e.g., 403 rate limit) */ }

        // 2) Fallback: direct commit from a known repository
        if (!found) {
          try {
            const commitsRes = await fetch(`https://api.github.com/repos/${GITHUB_REPO}/commits?per_page=1`, HEADERS); // latest commit for known repo
            if (commitsRes.ok) {
              ...
              if (c0) {
                ...
                const createdAt = c0?.commit?.author?.date || new Date().toISOString(); // timestamp
                const link = c0?.html_url || `https://github.com/${repo}/commit/${c0?.sha}`; // link
                found = { sha, message, repo, createdAt, link }; // normalized
              }
            }
        ...
        try { if (found) localStorage.setItem("lastCommitCache", JSON.stringify({ at: Date.now(), data: found })); } catch {} // cache when available
        if (!cancelled) setErrors(e => ({ ...e, commit: found ? null : "Rate limited — showing cached or nothing" })); // friendly error
      } catch (err) { // handle errors
        console.warn("GitHub fetch warning:", err); // log as warning to avoid noisy overlays
        if (!cancelled) setErrors(e => ({ ...e, commit: "Failed to load commit" })); // record error
      } // end try/catch
    } // end fetchCommit
    try { // try to hydrate from cache
      const raw = localStorage.getItem("lastCommitCache"); // read cache
      if (raw) { // if present
        const cached = JSON.parse(raw); // parse JSON
        if (cached?.data && Date.now() - cached.at < 10 * 60 * 1000) { // valid within 10 minutes
    ...
    return () => { cancelled = true; clearInterval(id); }; // cleanup when unmounting
  }, []); // run once

  // animation hook placeholder (previous background animation removed)
  useEffect(() => { // run once on mount to set up plane
  ...
  return ( // JSX output
    <main className="main minimal"> {/* main container with minimal style */}
      <h1 className="minimal-title"> {/* big site title */}
      ...
      <div className="link-row"> {}
        ...
        <a href="https://www.last.fm/user/th3mujd11">Last.fm</a> {/* link to web CTF challenge */}
      </div> {}
      ...
      <div className="kv-row"> {}
        ...
        <span className="kv-value"> {}
          {track ? <a href={track.url} target="_blank" rel="noreferrer">{track.name} — {track.artist}</a> : (errors.track ?? "Loading...")} {/* track link or loading */}
        </span> {}
      </div> {}

      <div className="kv-row"> {/* last commit row */}
        <span className="kv-key">Last Commit</span> {}
        <span className="kv-value"> {}
          {commit ? (
            <>
              <a href={`https://github.com/${commit.repo}`} target="_blank" rel="noreferrer">{commit.repo}</a> {/* repo */}
              {" — "}
              <a href={commit.link} target="_blank" rel="noreferrer">{commit.sha}</a> {/* sha */}
      ...
      <section className="blurb"> {}
        ...
        <p className="blurb-text">
          My projects range from “kind of useful” to “why did I even build this,” and I wouldn’t have it any other way. {/* instruction text */}
        </p> {}
      </section> {}
      ...
      <section className="section"> {}
        ...
        <ul className="thanks-list"> {}
          <li><a href="https://ctftime.org/team/287720" target="_blank" rel="noreferrer">Team 3xh4ck5</a> — since 2024</li> {/* team 1 with since 1980 */}
          <li><a href="https://github.com/scr1ptk1dd13s" target="_blank" rel="noreferrer">Team $scr1pt_K1dd13$</a> — since 2025</li> {/* team 2 with since 1980 */}
        </ul> {}
      </section> {}

      <section className="section"> {/* thanks section */}
        <h2 className="section-h2">Special Thanks:</h2> {/* heading */}
        <ul className="thanks-list"> {/* vertical bulleted list */}
          {/* replace the sample names below with your own */}
          <li><a href="https://github.com/Costinteo" target="_blank" rel="noreferrer">Sunbather</a> - for teaching me the basic concepts used in cybersecurity when i was a noob</li> {/* person 1 */}
          <li><a href="https://github.com/ItzDavi" target="_blank" rel="noreferrer">Agent Perry</a> - for welcoming me in his CTF team, introducing me basically to a new world</li> {/* person 2 */}
          <li><a href="https://github.com/C0mm4nd3rX" target="_blank" rel="noreferrer">CommanderX</a> - my best friend who stood by my side and helped me with the stupidest project ideas</li> {/* person 3 */}
        </ul> {/* end bulleted list */}
      </section> {/* end section */}

      <footer className="footer-stick"> {/* sticky footer at bottom edge */}
        © th3mujd11 2025 {/* exact footer text as requested */}
      </footer> {/* end footer */}
