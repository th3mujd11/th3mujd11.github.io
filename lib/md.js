...
export function markdownToHtml(md) { // accepts markdown text
  ...
  let text = md.replace(/\r\n?/g, "\n"); // unify newlines to \n
  // handle fenced code blocks with optional language: ```lang\n...``` and also ```` fences
  const codeBlocks = []; // store code block contents to re-insert later
  // First, handle four-backtick fences commonly used to nest backticks
  text = text.replace(/````([a-zA-Z0-9_-]+)?\n([\s\S]*?)````/g, (_m, lang, code) => {
    codeBlocks.push({ code, lang: (lang || "").toLowerCase() });
    return `{{{CODE_${codeBlocks.length - 1}}}}`;
  });
  // Then handle standard triple backtick fences
  text = text.replace(/```([a-zA-Z0-9_-]+)?\n([\s\S]*?)```/g, (_m, lang, code) => { // match triple backtick blocks
    codeBlocks.push({ code, lang: (lang || "").toLowerCase() }); // store raw code and language
  ...
  let inBlockquote = false; // whether inside a <blockquote>
  ...
  function closeBlockquote() { // helper to close blockquote
  ...
  for (let i = 0; i < lines.length; i++) { // iterate line by line
    ...
    if (/^\s*$/.test(line)) { // blank line
      ...
      closeBlockquote(); // close blockquote on blank line
    ...
    if (m) { // if heading
      ...
      closeBlockquote(); // close blockquote before heading
    ...
    if (/^\d+\.\s+/.test(line)) { // match ordered list item
      if (!inOl) { closeLists(); closeBlockquote(); html += "<ol>"; inOl = true; } // open <ol> if not already
    ...
    if (/^[\-*]\s+/.test(line)) { // match unordered list item
      if (!inUl) { closeLists(); closeBlockquote(); html += "<ul>"; inUl = true; } // open <ul> if not already
...
function inlineFormat(s) { // accepts raw text line
  ...
  // images ![alt](src) — handle before links to avoid conflict
...
// Rewrite some common image sources to render correctly
function rewriteImageSrc(src) {
  // If it's a GitHub blob link, append ?raw=1 to get the actual image bytes
  if (/^https?:\/\/github\.com\//i.test(src) && /\/blob\//.test(src)) {
    return src.includes('?') ? `${src}&raw=1` : `${src}?raw=1`;
  }
  return src; // otherwise, leave untouched
}
